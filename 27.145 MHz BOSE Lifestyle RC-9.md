...
In the espfuchs project someone seems to use the code as a [frequency generator](https://github.com/jokrug/espfuchs/blob/master/user/oscillator.c). I wonder if 25.145 MHz could be generated this way.

I have not tried this yet due to the lack of knowledge on how to generate the required signal. There is a `esp8266_bosecontrol.ino` sketch below but I did not get it to compile yet. Please let me know if you do.
...
* 3-pin "System control" (this we could possibly emulate using an Arduino if we knew the protocol - please let me know if you do, e.g., measure it with an oscilloscope and if signals are below 5V check with a Logic Analyzer) (3.5 mm headphones-like connector)
